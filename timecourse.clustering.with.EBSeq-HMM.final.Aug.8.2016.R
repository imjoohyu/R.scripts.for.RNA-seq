...
CountTable = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/mega_RNA-seq_count_of_all_samples_Apr_2016.txt", header=T) #Raw count data (17558 x 105)
...
for (j in 1:length(list.of.bacteria.name)){
    ...
    gene.list.for.a.given.path = GetConfidentCalls(EBSeqHMMGeneOut, FDR=.05, cutoff=0.75, OnlyDynamic=TRUE,Paths=NULL) #Obtain confident gene calls for classifying genes into expression paths, PP>=0.5 is most likely path
    ...
    write.table(de.list, file=paste("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_",list.of.bacteria.name[j],".txt", sep=""), quote=F)
}
...
list.of.M.luteus.genes = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_M.luteus.txt", header=T)
list.of.E.coli.genes = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_E.coli.txt", header=T)
list.of.S.mar.type.genes = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_S.mar.type.txt", header=T)
list.of.E.fae.live.genes = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_E.fae.live.txt", header=T)
list.of.P.rett.live.genes = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_P.rett.live.txt", header=T)
list.of.Ecc15.genes = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_Ecc15.txt", header=T)
list.of.E.fae.heatkilled.genes = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_E.fae.heatkilled.txt", header=T)
list.of.P.rett.heatkilled.genes = read.table("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_for_P.rett.heatkilled.txt", header=T)
...
for (g in 1:length(list.of.genes)){
    ...
    for (h in 1:dim(sample.gene)[1]){
        ...
        if (g == 2){ #E.coli
            one = mean(a[c(1:3),1]); two = mean(a[c(4:6),1]); three = mean(a[c(7:8),1]); four = mean(a[c(9:11),1])
        }
        else{ #Everything else
            one = mean(a[c(1:3),1]); two = mean(a[c(4:6),1]); three = mean(a[c(7:9),1]); four = mean(a[c(10:12),1])
        }
    ...
    write.table(final, file = paste("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/EBSeqHMM_DE_results_checking_cutoff_for_", list.of.bacteria.name[g], ".txt", sep=""), quote=F)
}

#Comments (8/22/2016):
#The results showed that the call made by EBSeq-HMM do not always match with the prediction from simple comparison (my script) of time points.
#This was the case even for the genes that had higher probability of prediction by EBSeq-HMM.
...
#Filter out those with P<0.5 and put together expression values with gene name and cluster types
number.of.cluster = c()
for (s in 1:length(list.of.bacteria.name)){ 
    ...
    mean.bacteria.log2 = log2(mean.bacteria + 1) #transform data for plotting
    most.likely.paths.bacteria = cbind(most.likely.paths,mean.bacteria.log2)
    ...
    write.table(most.likely.paths.bacteria, file = paste("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/clustering/EBSeq-HMM_Aug_2016/",list.of.bacteria.name[s],".with.most.liekly.paths.txt",sep=""), quote=F, row.names =T, col.names=T)
...
print(grid.arrange(q1,q2,q3,q4,q5,q6, ncol=4, newpage=T))
...
grid.arrange(q1,q2,q3,q4,q5,q6, ncol=4, newpage=T)

#S.mar.type - 8 clusters
...
q8 = ggplot(data = S.mar.type.reshape.c8, aes(x=S.mar.type.reshape.c8$time, y = S.mar.type.reshape.c8$log2.exp.value, group = S.mar.type.reshape.c8$ids)) + geom_line() + theme_classic()+xlab("Time") + ylab("log2(expression)")
...
q8 = ggplot(data = E.fae.live.reshape.c8, aes(x=E.fae.live.reshape.c8$time, y = E.fae.live.reshape.c8$log2.exp.value, group = E.fae.live.reshape.c8$ids)) + geom_line() + theme_classic()+xlab("Time") + ylab("log2(expression)")
grid.arrange(q1,q2,q3,q4,q5,q6,q7,q8, ncol=4, newpage=T)
...
#E.faecalis heatkilled - 4 clusters
...
grid.arrange(q1,q2,q3,q4, ncol=4, newpage=T)

#P.rettgeri heatkilled - 6 clusters
...
q6 = ggplot(data = P.rett.heatkilled.reshape.c6, aes(x=P.rett.heatkilled.reshape.c6$time, y = P.rett.heatkilled.reshape.c6$log2.exp.value, group = P.rett.heatkilled.reshape.c6$ids)) + geom_line() + theme_classic()+xlab("Time") + ylab("log2(expression)")
print(grid.arrange(q1,q2,q3,q4,q5,q6, ncol=4, newpage=T))
