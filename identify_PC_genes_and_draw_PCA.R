#Identify PC genes and draw a PCA plot
#April 18th 2017
#Joo Hyun Im (ji72)
...
#for 12hr Gram-negative live and heatkilled infections only
CountTable <- CountTable[,c(11,14,20,23,27,28,29,33,46,49,55,58,62,63,64,68,81,84,90,93,97,98,99,103)]
design <- design[c(11,14,20,23,27,28,29,33,46,49,55,58,62,63,64,68,81,84,90,93,97,98,99,103),]
#for 12hr Gram-positive live and heatkilled infections only
CountTable <- CountTable[,c(8,17,26,30,43,52,61,65,78,87,96,100)]
design <- design[c(8,17,26,30,43,52,61,65,78,87,96,100),]
#for 132hr 10 live infections + heatkilled
CountTable <- CountTable[,c(10,13,16,19,22,25,32,35,45,48,51,54,57,60,67,70,80,83,86,89,92,95,102,105)]
design <- design[c(10,13,16,19,22,25,32,35,45,48,51,54,57,60,67,70,80,83,86,89,92,95,102,105),]
...
ggplot(PCs.with.design, aes(x=X.ggplot, y=Y.ggplot)) + geom_point(size=8, aes(color = factor(treatment))) + scale_color_manual(values = cols, breaks=c("M.luteus", "E.coli", "S.marcescens Type","Ecc15","P.rettgeri","P.rettgeri heatkilled","E.faecalis","E.faecalis heatkilled","S.aureus","P.sneebia","S.marcescens Db11","P.entomophila"), name="Infection Conditions") + xlab(paste("PC1 (", variance[1], "%)",sep="")) + ylab(paste("PC2 (", variance[2], "%)",sep="")) + theme_bw() + theme(axis.title = element_text(face = "bold", size = 20), axis.text = element_text(face = "bold", size = 20), legend.title = element_text(size = 20), legend.text = element_text(size=20), panel.grid.major.x = element_blank(), panel.grid.major.y = element_blank())
#dev.off()


#Get the expression values for these genes
expression_data = read.table("edgeR_results_with_cpm_filter_all_UCs_Nov_2015/edgeR_basic_comparison_all_genes_FC.txt", header=T)
pca.rotation_ordered_df = data.frame(pca.rotation_ordered)
expression_data_subset = expression_data[match(rownames(pca.rotation_ordered_df), expression_data[,1]),]
