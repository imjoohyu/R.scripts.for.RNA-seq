...
PC1_top_100_genes = read.table("PCA_using_DESeq2/genes.loaded.on.PC1.for.live.and.heatkilled.12hr_top100_reannotated.txt", header=T, stringsAsFactors = F, sep="\t")
...
#Label = c(rep(other_colors[1],38),rep(other_colors[10],12),rep(other_colors[3],5),rep(other_colors[4],7),rep(other_colors[5],5),rep(other_colors[6],5),rep(other_colors[7],3), other_colors[8], other_colors[9], rep(other_colors[11],23))
Label = c(rep("#a6611a",38),rep("#ffffbf",12),rep(other_colors[3],5),rep("#dfc27d",7),rep(other_colors[4],5),rep(other_colors[5],5),rep("#018571",3), other_colors[6], "#80cdc1", rep(other_colors[7],23)) #to be consistent with the cor gene function colors


pdf(file="PCA_using_DESeq2/heatmap_of_PC1_genes_re-annotated_without_function_label.pdf", height=25, width=25) 
#heatmap.2(as.matrix(expression_data_subset_with_name), Rowv=FALSE, Colv=FALSE, density.info="column", trace="none", symm=F, scale="none", col=my_palette, srtCol=45, key=T, keysize = 1, cexCol=0.76, cexRow=0.5, RowSideColors = Label, key.xlab="Expression") #gives a heatmap with an order of virulence level
...
#without dendrogram so that I can get the x-axis label
heatmap.2(as.matrix(expression_data_subset_with_name), Rowv=FALSE,trace="none", symm=F, scale="none", col=my_palette, srtCol=45, key=T, keysize = 0.8, cexCol=1, cexRow=0.3, RowSideColors = Label, key.xlab="Expression") #gives a heatmap with an order based on PCA clustering



#Draw a heatmap of genes with the information on whether they are Imd targets or now. The goal is to show that the conditions on one end of PC1 have higher expression of IMD targets than conditions on the other end of PC1. 3/30/2017
PC1_top_100_genes = read.table("PCA_using_DESeq2/genes.loaded.on.PC1.for.live.and.heatkilled.12hr_top100_reannotated_w_rel_target_status.txt", header=T, sep="\t")

#expression_data = read.table("edgeR_results_with_cpm_filter_all_UCs_Nov_2015/edgeR_basic_comparison_all_genes_FC.txt", header=T) #FC, not count data, 11911 x 64
function_order=c("Y", "N","NA")
PC1_top_100_genes = PC1_top_100_genes[order(match(PC1_top_100_genes$rel_target_by_microarray, function_order)),] #group by function
expression_data_subset = expression_data[match(PC1_top_100_genes[,1], expression_data[,1]),]
expression_data_subset_with_name = expression_data_subset[,c(3,9,15,21,27,33,39,45,47,49,51,53,59)] #for FC
#expression_data_subset_with_name = expression_data_subset[,c(4,7,10,13,16,19,22,25,26,27,28,29,32)] #for count
...
pdf(file="/Users/JooHyun/Desktop/t.pdf", height=20, width=20) 
#par(mar=c(5,0,0,10)+0.1) #sets the bottom, left, top and right margins -- doesn't work
heatmap.3(as.matrix(expression_data_subset_with_name), Rowv=FALSE, density.info="none", dendrogram="column", trace="none", symm=F, scale="none", col=my_palette, key=F, keysize = 1, cexCol=1.2, cexRow=1, RowSideColors=rlab, RowSideColorsSize=92) #gives a heatmap with an order based on PCA clustering
dev.off()


#heatmap for PC2 genes
rm(list=ls(all=TRUE)) #delete any previous entry
setwd("/Users/JooHyun/Dropbox/Cornell/Lab/Projects/Mega_RNA-seq/")
...
#Are PC1 genes the Imd targets? (comparing the PC1 genes to the microarray data from De Gregorio et al 2002 -- see Evernote 3/29/17)
