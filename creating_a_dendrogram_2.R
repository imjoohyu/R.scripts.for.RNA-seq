...
data_table_FC_only_12hr = data_table_FC_only[,c(1,4,7,10,13,16,19,22,23,24,25,26,29)] #12hr sample only, 11911 x 13, OR
data_table_FC_only_12hr = data_table_FC_only[,c(4,7,10,13,16,19,22,23,24,25,26,29)] #12hr sample only minus the sterile wound, 11911 x 12
data_table_FC_only_12hr = t(data_table_FC_only_12hr) #13 x 11911 or 23 x 11911

hc = hclust(dist(data_table_FC_only_12hr))
dd <- as.dendrogram(hc)
plot(hc, hang = -1,lwd = 5); plot(rev(hc), hang = -1)
...
#Dendrogram of just live bacteria (3/31/2017)
#Dendrogram based on 12hr sample >> This pattern goes well with the PCA pattern as expected (12/9/2016)
odd_index = seq(3,64,2)
data_table_FC_only = data_table[,c(odd_index)]; rownames(data_table_FC_only) = data_table[,1]
data_table_FC_only_12hr = data_table_FC_only[,c(4,7,10,13,16,19,22,23,24,25)] #12hr sample only, 11911 x 13
data_table_FC_only_12hr = t(data_table_FC_only_12hr) #10 x 11911
hc = hclust(dist(data_table_FC_only_12hr))
dd <- as.dendrogram(hc)
