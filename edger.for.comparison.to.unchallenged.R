...
normalized.counts = cpm(d, normalized.lib.sizes=F) #Getting moderated log-counts-per-million from normalized counts from all conditions -- updated on 5/16/2017
...
for (i in 1:length(gene.id)[1]) {
    gene.to.compare = as.character(gene.id[i]); checking.against.full.list <- full.list[which(full.list$gene_id == gene.to.compare),]
