...
rownames(data_table_FC_only_12hr) = c("CecA1","CecA2","CecB","CecC","Jra/AP1","kay","Punch","Dpt","Dro","AttA","Imd","Rac2","Rel","Iap2","Helicase89B","Tak1","TotA","TotM","PGRP-SC1b","Listericin","Caspar","DptB","pirk","PGRP-LA","PGRP-LC","pGRP-LB","AttD","dFADD/BG4","kenny","AttC","AttB1", "PGRP-SC2","edin")

for (i in 1:dim(data_table_FC_only_12hr)[1]){
    print(imd_genes[which(imd_genes$gene_id == rownames(data_table_FC_only_12hr)[i]),3])
}
...
heatmap.2(as.matrix(data_table_FC_only_12hr), Rowv=FALSE, density.info="none", dendrogram="column", trace="none",symm=F, scale="none", col=my_palette, margins = c(12,12), srtCol=45)
dev.off()
