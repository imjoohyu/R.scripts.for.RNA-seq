...
get_gene_list_and_data = function(gene_list, sample_list){
    ...
    for (i in 1:length(list_of_genes_only)){
        ...
        FC_data_picked = rbind(FC_data_picked, FC_data_set)
    }
    row.names(FC_data_picked) = as.vector(list_of_genes$gene_name)
    return(as.matrix(FC_data_picked))
}
...
A1_data = get_gene_list_and_data("edgeR_prev_infected_vs_present_infected_genes.that.behave.the.same.within.each.gram.infection.but.differently.between.gram.infections.txt", c(9,11,13,27,29,31,15,17,19,21,23,25,33,35,37,39,41,43))
...
A2_data = get_gene_list_and_data("edgeR_prev_infected_vs_present_infected_common_genes.that.behave.differently.between.live.and.heatkilled.txt", c(27,29,31,53,55,57,33,35,37,59,61,63))
...
#M.luteus, E.faecalis, E.coli, S.marcescns Type, P.rettgeri, Ecc15
A3_data = get_gene_list_and_data("edgeR_prev_infected_vs_present_infected_genes.that.behave.the.same.across.infection.conditions.txt", c(9,11,13,27,29,31,15,17,19,21,23,25,33,35,37,39,41,43))
